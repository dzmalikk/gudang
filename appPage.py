...
class AppPage(tk.Frame):
	...
	def create_left_frame(self):
		self.left_frame = tk.Frame(self, bg="skyblue")
	...
	def create_right_frame(self):
		self.right_frame = tk.Frame(self, bg="gold", width=2*self.settings.width//3) # 3434
	...
	def create_left_header(self):
		...
		self.search_box_frame = tk.Frame(self.left_frame, bg="red", width=frame_w, height=frame_h//4)
		self.search_box_frame.pack(fill="x") # 5656

		self.entry_search_var = tk.StringVar()
		self.entry_search = tk.Entry(self.search_box_frame, bg="white", fg="black", font=("Arial", 12), textvariable=self.entry_search_var) # 5656
		self.entry_search.grid(row=0, column=0)

		self.button_search = tk.Button(self.search_box_frame, bg="cyan", fg="pink", text="Find", font=("Arial", 12), command=self.clicked_search_btn) # 5656
	...
	def show_list_items_in_listbox(self):
		...
		# 	for code, info in contact.items():
		# 		item_name = f"{info['f_name']} {info['l_name']}"
		# 		self.items_list_box.insert("end", item_name)
		for index in self.items_index:
			item = items[index]
			for code, info in item.items():
				item_name = f"{info['nama_barang']}"
				self.items_list_box.insert("end", item_name)
	...
	def create_left_content(self):
		...
		self.items_list_box = tk.Listbox(self.left_content, bg="pink", fg="black", font=("Arial", 12), height=frame_h) # 3434
	...
	def clicked_item_inListBox(self, event):
		if not self.update_mode:
			...
			for number_id, info in self.current_item.items():
				code = number_id
				item_name = info['nama_barang']
				tanggal_masuk = info['tanggal_masuk']
				jumlah = info['jumlah']
				harga = info['harga']

			self.full_name_label.configure(text=item_name)
			self.table_info[0][1].configure(text=code)
			self.table_info[1][1].configure(text=tanggal_masuk)
			self.table_info[2][1].configure(text=jumlah)
			self.table_info[3][1].configure(text=harga)
	...
	def create_detail_right_header(self):
		...
		self.detail_header = tk.Frame(self.right_header, width=frame_w, height=frame_h, bg="darkorange")
		self.detail_header.grid(row=0, column=0, sticky="nsew")

		data_dictionary = list(self.current_item.values())[0]
		item_name = f"{data_dictionary['nama_barang']}"
		self.virt_img = tk.PhotoImage(width=1, height=1)
		self.full_name_label = tk.Label(self.detail_header, text=item_name, font=("Arial", 30), width=frame_w, height=frame_h, image=self.virt_img, compound='c', bg="darkorange") # 3434
	...
	def create_detail_right_content(self):
		...
		self.detail_content = tk.Frame(self.right_content, width=frame_w, height=frame_h, bg="gold") # 4545
		...
		for row in range(rows):
			...
			for column in range(columns):
				label = tk.Label(self.detail_content, text=info[row][column], font=("Arial", 12), bg="gold") # 4545
				aRow.append(label)
				if column == 0:
					sticky = "e"
				else:
	...
	def create_detail_right_footer(self):
		...
		self.detail_footer = tk.Frame(self.right_footer, width=frame_w, height=frame_h, bg="gold") # 4545
		self.detail_footer.grid(row=0, column=0, sticky="nsew")

		features = ['Update', 'Delete', 'Add New','Exit']
		commands = [self.clicked_update_btn, self.clicked_delete_btn, self.clicked_add_new_btn, self.clicked_exit_btn]
		self.buttons_features = []
		for feature in features:
			button = tk.Button(self.detail_footer, text=feature, bg="yellow", fg="darkorchid1", bd=0, font=("Arial", 12, "bold"), command=commands[features.index(feature)])
	...
	def clicked_update_btn(self):
		...
		self.detail_update_content = tk.Frame(self.right_content, width=frame_w, height=frame_h, bg="gold") #6767
		self.detail_update_content.grid(row=0, column=0, sticky="nsew")

		for number_id, info in self.current_item.items():
			info = [
				['Nama Barang :', info['nama_barang']],
				['Tanggal Masuk :', info['tanggal_masuk']],
				['ID Barang :', number_id],
				['Jumlah :', info['jumlah']],
				['Harga :', info['harga']]
		...
		for row in range(rows):
			...
			for column in range(columns):
				if column == 0:
					label = tk.Label(self.detail_update_content, text=info[row][column], font=("Arial", 12), bg="gold") #6767
					sticky = "e"
					aRow.append(label)
					label.grid(row=row, column=column, sticky=sticky)
				else:
		...
		self.detail_update_footer = tk.Frame(self.right_footer, width=frame_w, height=frame_h, bg="gold") # 6767
		self.detail_update_footer.grid(row=0, column=0, sticky="nsew")

		features = ['Save', 'Cancel','Exit']
		commands = [self.clicked_save_item_btn, self.clicked_cancel_item_btn, self.clicked_exit_btn]
		self.buttons_features = []
		for feature in features:
			button = tk.Button(self.detail_update_footer, text=feature, bg="yellow", fg="darkorchid1", bd=0, font=("Arial", 12, "bold"), command=commands[features.index(feature)]) #6767
			button.grid(row=0, column=features.index(feature), sticky="nsew", padx=20)
			self.buttons_features.append(button)
	...
	def clicked_delete_btn(self):
		...
		confirm = msg.askyesnocancel('Item Delete Confirmation', 'Are you sure to delete this item ?')
		index  = self.last_current_item_index
		if confirm:
			self.settings.items.pop(index)
			self.settings.save_data_to_json()
			self.last_current_item_index = 0
			self.current_item = self.settings.items[self.last_current_item_index]
	...
	def clicked_add_new_btn(self):
		...
		self.detail_add_new_header = tk.Frame(self.right_header, width=frame_w, height=frame_h, bg="darkorange") # 7878
		self.detail_add_new_header.grid(row=0, column=0, sticky="nsew")

		self.virt_img = tk.PhotoImage(width=1, height=1)
		self.add_new_label = tk.Label(self.detail_add_new_header, text="Tambah Barang Baru", font=("Arial", 30), width=frame_w, height=frame_h, image=self.virt_img, compound='c', bg="darkorange") # 7878
		...
		self.detail_add_new_content = tk.Frame(self.right_content, width=frame_w, height=frame_h, bg="gold") # 7878
		...
		for row in range(rows):
			...
			for column in range(columns):
				if column == 0:
					label = tk.Label(self.detail_add_new_content, text=info[row][column], font=("Arial", 12), bg="gold") # 7878
					sticky = "e"
					aRow.append(label)
					label.grid(row=row, column=column, sticky=sticky)
				else:
		...
		self.detail_add_new_footer = tk.Frame(self.right_footer, width=frame_w, height=frame_h, bg="gold") # 7878
		self.detail_add_new_footer.grid(row=0, column=0, sticky="nsew")

		features = ['Save', 'Cancel', 'Exit']
		commands = [self.clicked_save_add_new_item_btn, self.clicked_cancel_add_new_item_btn, self.clicked_exit_btn]
		self.buttons_features = []
		for feature in features:
			button = tk.Button(self.detail_add_new_footer, text=feature, bg="yellow", fg="darkorchid1", bd=0, font=("Arial", 12, "bold"), command=commands[features.index(feature)]) # 7878
			button.grid(row=0, column=features.index(feature), sticky="nsew", padx=20)
			self.buttons_features.append(button)
	...
	def clicked_save_item_btn(self):
		...
		confirm = msg.askyesnocancel('Item Save Confirmation', 'Are you sure to update this item ?')
		
		index = self.last_current_item_index
		if confirm:
			nama_barang = self.entry_update_item_vars[0].get()
			tanggal_masuk = self.entry_update_item_vars[1].get()
			code = self.entry_update_item_vars[2].get()
			jumlah = self.entry_update_item_vars[3].get()
	...
	def clicked_exit_btn(self):
		respond = msg.askyesnocancel("Quit", "Are you sure want to quit the program ?")
		if respond:
			sys.exit()
	...
	def clicked_search_btn(self):
		...
		if item_search:
			...
			index_counter = 0
			for item in items:
				for number_id, info in item.items():
					if item_search in number_id:
						print(number_id)
						self.items_index.append(index_counter)
					elif item_search in info['nama_barang']:
						print(info['nama_barang'])
						self.items_index.append(index_counter)
					elif item_search in info['tanggal_masuk']:
						print(info['tanggal_masuk'])
						self.items_index.append(index_counter)
				index_counter += 1
	...
	def clicked_save_add_new_item_btn(self):
		...
		confirm = msg.askyesnocancel('Item Save Confirmation', 'Are you sure to add this item ?')
		...
		if confirm:
			...
			self.settings.items.append(new_item)
			self.settings.save_data_to_json()
		index = len(self.settings.items)-1
		self.last_current_item_index = index
		self.current_item = self.settings.items[self.last_current_item_index]
